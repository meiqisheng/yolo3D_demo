...
def detect3d(
    ...
    if(video_save):
        ...
        fps = 20  # 帧数
        width, height = int(852), int(480)  # 宽高
        out = cv2.VideoWriter('runs/result.mp4', fourcc, fps, (width, height))  # 写入视频
            # loop images
        for i, img_path in enumerate(imgs_path):
            ...
            for img, det_s in zip(imgs, dets):
                for det in det_s:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    result_list.append(Bbox(bbox, label))

            # Print time (inference-only)
            LOGGER.info(f'{s}Done. ({t3 - t2:.3f}s)')
        bbox_list.append(result_list)
    # Print results
...
def parse_opt():
    ...
    parser.add_argument('--video_save', action='store_true', help='Save Video result')
